...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    {
		sLog->outError(LOG_FILTER_PLAYER, "bot_ai::SetBotOwner(): bot %s (id: %u) has master %s while trying to set to %s...",
    ...
    if (IsMinionAI())
    {
        ...
        if (mgr->AddBot(me, takeMoney) & BOT_ADD_FATAL)
        {
            ...
            if (_ownerGuid)
            {
				sLog->outError(LOG_FILTER_PLAYER, "bot_ai::FindMaster(): %s's master %s (guid: %u) is found but bot failed to set owner (fatal)! Unbinding bot temporarily (until server restart)...",
					me->GetName(), newowner->GetName(), newowner->GetGUIDLow());
                _ownerGuid = 0;
            }
...
SpellCastResult bot_ai::CheckBotCast(Unit* victim, uint32 spellId, uint8 botclass) const
{
    ...
    if ((int32)me->GetPower(Powers(spellInfo->PowerType)) < (int32)spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask(), me->GetSpellPowerEntryBySpell(spellInfo)))
        return SPELL_FAILED_NO_POWER;
    ...
    switch (botclass)
    {
        ...
        case BOT_CLASS_DEATH_KNIGHT:
            break;
        default:
			sLog->outError(LOG_FILTER_PLAYER, "CheckBotCast(): Unknown bot class %u", botclass);
            break;
    }
...
void bot_minion_ai::BuffAndHealGroup(Player* gPlayer, uint32 diff)
{
    ...
    if (HasRole(BOT_ROLE_HEAL))
    {
        for (uint8 i = 0; i != TARGETICONCOUNT; ++i)
        {
            ...
            {
                if (/*ObjectGuid*/uint64 guid = pGroup->GetTargetIcons()[i])//check this one
                {
...
void bot_minion_ai::RezGroup(uint32 REZZ, Player* gPlayer)
{
    ...
    if (!pGroup)
    {
        ...
        if (master->IsResurrectRequested()) return;
...
void bot_minion_ai::_getBotDispellableAuraList(Unit* target, Unit* caster, uint32 dispelMask, DispelChargesList& dispelList) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        AuraApplication* aurApp = itr->second->GetApplicationOfTarget(target->GetGUID());
        if (!aurApp)
            continue;

        if (itr->second->GetSpellInfo()->GetDispelMask() & dispelMask)
        {
            ...
            if (itr->second->GetSpellInfo()->IsRankOf(sSpellMgr->GetSpellInfo(34914)))
                continue;

            uint8 charges = (itr->second->GetSpellInfo()->AttributesEx7 & SPELL_ATTR7_DISPEL_CHARGES) ? itr->second->GetCharges() : itr->second->GetStackAmount();
            if (charges > 0)
                dispelList.push_back(std::make_pair(itr->second, charges));
        }
    }
}
//protected
bool bot_ai::HasAuraName(Unit* unit, uint32 spellId, ObjectGuid casterGuid, bool exclude) const
{
    ...
    if (!spellInfo)
    {
		sLog->outError(LOG_FILTER_PLAYER, "bot_ai::HasAuraName(uint32): No SpellInfo found for spell %u! [class:%u]", spellId, GetBotClass());
        return false;
    }
...
void bot_ai::_listAuras(Player* player, Unit* unit) const
{
    ...
    ch.PSendSysMessage("ListAuras for %s (class: %u), %s", unit->GetName(), bot_pet_player_class, botstring.str().c_str());
...
void bot_minion_ai::SetStats(bool force, bool shapeshift)
{
    ...
    switch (myclass)
    {
        ...
        default:
            sLog->outError(LOG_FILTER_PLAYER, "minion_ai: *etStats():Init - unknown bot class %u, real class: %u, _botclass: %u", myclass, GetPlayerClass(), _botclass);
...
void bot_pet_ai::SetStats(bool force, bool /*unk*/)
{
    ...
    switch (petType)
    {
        ...
        default:
			sLog->outError(LOG_FILTER_PLAYER, "pet_ai: *etStats():Init - unknown pet type %u", petType);
    ...
    if (ap_mod > 0.f)//do not bother casters
    {
        switch (m_creatureOwner->GetBotClass())
        {
            ...
            default: //some weird class or NYI
				sLog->outError(LOG_FILTER_PLAYER, "*etStats():Damage - unknown bot owner class %u", uint8(m_creatureOwner->GetBotClass()));
                value = 0.0f;
                break;
        }
    ...
    if (spp_mod > 0.f)
    {
        switch (m_creatureOwner->GetBotClass())
        {
            ...
            case BOT_CLASS_HUNTER:
                ...
                break;
            default: //some weird class or NYI
				sLog->outError(LOG_FILTER_PLAYER, "*etStats():Spellpower - unknown bot owner class %u", uint8(m_creatureOwner->GetBotClass()));
                value = 0.f;
                break;
        }
...
bool bot_ai::IsInBotParty(Unit const* unit) const
{
    ...
    if (Group* gr = master->GetGroup())
    {
        ...
        for (uint8 i = 0; i != TARGETICONCOUNT; ++i)
            if (_healTargetIconFlags & GroupIconsFlags[i])
                if (/*ObjectGuid*/uint64 guid = gr->GetTargetIcons()[i])//check this one
...
bool bot_ai::RefreshAura(uint32 spellId, int8 count) const
{
    ...
    if (!spellInfo)
    {
		sLog->outError(LOG_FILTER_PLAYER, "bot_ai::RefreshAura(): No SpellInfo for spell %u! Bot - %s (class: %u, entry: %u)", spellId, me->GetName(), _botclass, me->GetEntry());
        return false;
    }
    ...
    if (IsMinionAI())
    {
        switch (botOrPetType)
        {
            ...
            case BOT_CLASS_BM:
                break;
            default:
                TC_LOG_ERROR(LOG_FILTER_PLAYER, "bot_ai: CheckAttackTarget() - unknown bot class %u", botOrPetType);
                return false;
        }
    }
    else
    {
        switch (botOrPetType)
        {
            case PET_TYPE_IMP:
                ...
                break;
            default:
                TC_LOG_ERROR(LOG_FILTER_PLAYER, "bot_ai: CheckAttackTarget() - unknown pet type %u", botOrPetType);
                return false;
        }
    }
    ...
    if (IsMinionAI())
    {
        ...
        /*switch (_botclass)
        {
            ...
            case BOT_CLASS_BM:
                ...
                break;
            default:
                TC_LOG_ERROR(LOG_FILTER_PLAYER, "bot_ai: ApplyPassives() - unknown bot class %u for bot %s (id: %u)",
                    uint32(_botclass), me->GetName(), me->GetEntry());
                break;
        }*/
    ...
    else
    {
        /*switch (bot_pet_ai::GetPetType(me))
        {
            case PET_TYPE_VOIDWALKER:
                break;
            default:
                TC_LOG_ERROR(LOG_FILTER_PLAYER, "bot_ai: ApplyPassives() - unknown pet type %u for bot %s (id: %u)",
                    uint32(bot_pet_ai::GetPetType(me)), me->GetName(), me->GetEntry());
                break;
        }
		*/
        //RefreshAura(THREAT, 1 * IsTank()); //+43%
...
Unit* bot_minion_ai::FindAffectedTarget(uint32 spellId, ObjectGuid caster, float dist, uint8 hostile) const
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_PLAYER, "bot_ai::FindAffectedTarget(): hostile = %u while bot is free! Setting to 3...", hostile);
        hostile = 3;
    }
...
uint32 bot_ai::InitSpell(Unit const* caster, uint32 spell)
{
    ...
    if (!info)
    {
        TC_LOG_ERROR(LOG_FILTER_PLAYER, "InitSpell(): No SpellInfo found for spell %u", spell);
...
void bot_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        TC_LOG_ERROR(LOG_FILTER_PLAYER, "InitSpellMap(): No SpellInfo found for base spell %u [class:%u]", basespell, GetBotClass());
...
void bot_minion_ai::_OnHealthUpdate() const
{
    ...
    uint32 m_basehp = _basehp;//_classinfo->basehealth;
...
void bot_minion_ai::_OnManaUpdate(bool /*shapeshift*/)
{
	...
	float m_basemana = _basemana;// _classinfo->basemana;
    if (myclass == BOT_CLASS_BM)
...
void bot_minion_ai::_OnMeleeDamageUpdate(uint8 myclass) const
{
	...
	Unit::AuraEffectList const& mAPbyStat = me->GetAuraEffectsByType(SPELL_AURA_268); // old SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT);
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (auraname == SPELL_AURA_MOD_STAT || auraname == SPELL_AURA_MOD_PERCENT_STAT ||
            ...
            auraname == SPELL_AURA_268/* old SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT*/ || auraname == SPELL_AURA_MOD_ATTACK_POWER_OF_ARMOR ||
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_HIRE:
        {
            ...
            if (!reason)
            {
                if (_ownerGuid)
                {
                    ...
                    if (sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HighGuid::HIGHGUID_PLAYER, _ownerGuid), name))
                        ostr << name;
                    else
                        ostr << "unknown (" << _ownerGuid << ')';
                    BotSay(ostr.str().c_str(), player);
                    ChatHandler ch(player->GetSession());
                    ch.PSendSysMessage("%s will not join you until owner dismisses %s", me->GetName(), (me->getGender() == GENDER_MALE ? "him" : "her"));
                    break;
                }
        ...
        case GOSSIP_SENDER_DEBUG_ACTION:
        {
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case 6: //reload config
                {
                    ...
                    sLog->outInfo(LOG_FILTER_WORLDSERVER, "Re-Loading config settings...");
        ...
        case GOSSIP_SENDER_DEBUG:
        {
            ...
            ostr << "Bot: " << me->GetName()
                ...
                << "). owner: ";
            if (_ownerGuid && sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HighGuid::HIGHGUID_PLAYER, _ownerGuid), name))
...
void bot_minion_ai::SummonBotsPet(uint32 entry)
{
    ...
    master->SetMinion((Minion*)m_botsPet, true, PET_SLOT_UNK_SLOT);
...
bool bot_minion_ai::_canUseOffHand() const
{
    ...
    if (!_equips[1])
    {
        ...
        if (!proto)
            ...
            proto->SubClass == /*ITEM_SUBCLASS_WEAPON_FIST*/ITEM_SUBCLASS_WEAPON_FIST_WEAPON ||
...
bool bot_minion_ai::_canEquip(ItemTemplate const* item, uint8 slot, bool ignoreItemLevel) const
{
    ...
    else if (item->Class == ITEM_CLASS_ARMOR)
    {
        ...
        else if (item->SubClass == /*ITEM_SUBCLASS_ARMOR_MISC*/ITEM_SUBCLASS_ARMOR_MISCELLANEOUS)
        {
...
bool bot_minion_ai::_equip(uint8 slot, Item* newItem)
{
    ...
    {
        sLog->outError(LOG_FILTER_PLAYER,
            ...
            master->GetName(), master->GetGUIDLow(), me->GetName(), me->GetEntry(), proto->Name1.c_str(), proto->ItemId, newItem->GetGUIDLow());
        return false;
    }
...
void bot_minion_ai::_updateEquips(uint8 slot, Item* item)
{
    ...
    for (k = 0; k != BOT_INVENTORY_SIZE; ++k)
    {
        if (Item* botitem = _equips[k])
        {
			...
			stmt->setUInt32(++index, botitem->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 0)); // reforge Id
			stmt->setUInt32(++index, botitem->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 1)); // Transmogrification Id
			stmt->setUInt32(++index, botitem->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 2)); // itemUpgrade Id
...
bool bot_minion_ai::_resetEquipment(uint8 slot)
{
    ...
    {
        sLog->outError(LOG_FILTER_PLAYER, "minion_ai::_resetEquipment(): player %s (guidLow: %u) failed to reset equipment for bot %s (id: %u) in slot %u",
                master->GetName(), master->GetGUIDLow(), me->GetName(), me->GetEntry(), slot);
        return false;
    }
    return true;
}
...
void bot_minion_ai::ApplyItemBonuses(uint8 slot)
{
    ...
    ScalingStatValuesEntry const* ssv = /*proto->ScalingStatValue*/ ssd ? sScalingStatValuesStore.LookupEntry(ssd_level) : NULL;
    ...
    for (uint8 i = 0; i != MAX_ITEM_PROTO_STATS; ++i)
    {
        ...
        if (ssd && ssv)
        {
            ...
            val = (ssv->/*getssdMultiplier*/GetStatMultiplier(proto->InventoryType/*ScalingStatValue*/) * ssd->Modifier[i]) / 10000;
        }
        else
        {
	...
	*/
    ...
    if (GetPlayerClass() == BOT_CLASS_DRUID)
    {
        ...
        if (ssv)
        {
			...
			dpsMod = ssv->GetDPSAndDamageMultiplier(proto->SubClass, proto->Flags2 & ITEM_FLAGS_EXTRA_CASTER_WEAPON, &damageMultiplier);
			feral_bonus += ssv->getFeralBonus(proto->InventoryType);//ScalingStatValue);
        }
...
void bot_minion_ai::RemoveItemEnchantment(Item* item, EnchantmentSlot eslot, uint8 /*slot*/)
{
    ...
    for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
    {
        ...
        switch (enchant_display_type)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
				if (enchant_spell_id)
					me->RemoveAurasDueToItemSpell(item, enchant_spell_id);//(enchant_spell_id, item->GetGUID());
                break;
            case ITEM_ENCHANTMENT_TYPE_RESISTANCE:
                //Already removed in RemoveItemBonuses()
                break;
...
void bot_minion_ai::ApplyItemEquipSpell(Item* item, bool apply)
{
    ...
    for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
    {
		...
		if (apply)
			...
			me->RemoveAurasDueToItemSpell(item, spellInfo->Id);//spellInfo->Id, item->GetGUID());  // un-apply all spells, not only at-equipped
    }
}
...
void bot_minion_ai::InitFaction()
{
	...
	if (CreatureTemplate *cinfo = const_cast<CreatureTemplate*>(me->GetCreatureTemplate()))
	{
		cinfo->faction_A = faction;
		cinfo->faction_H = faction;
	}
}
...
void bot_minion_ai::InitEquips()
{
    ...
    if (!cnresult)
    {
        sLog->outError(LOG_FILTER_PLAYER, "bot_minion_ai::InitEquips(): Failed to initialize equips for bot %s (id: %u, guidLow: %u), not found in `characters_npcbots table`!!!",
            me->GetName(), me->GetEntry(), me->GetGUIDLow());
        ASSERT(false);
    }
...
{
    return (!_ownerGuid || _ownerGuid != master->GetGUIDLow() || !me->HasUnitTypeMask(UNIT_MASK_MINION));
...
void bot_minion_ai::BotJump(Position* pos)
{
    ...
    me->BotStopMovement();
    me->GetMotionMaster()->MoveJump(/**pos*/pos->GetPositionX(), pos->GetPositionY(), pos->GetPositionZ(), me->GetExactDist2d(pos->m_positionX, pos->m_positionY), 10.0f);
...
bool bot_minion_ai::UpdateImpossibleChase(Unit* target)
{
    ...
    if (!me->IsWithinDist(target, IsMelee() ? 50 : 75) || (me->GetDistance(target) < (IsMelee() ? 5 : 25)))
    {
		...
		me->GetMotionMaster()->MovePoint(me->GetMapId(), target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()); //*target, false);
        return true;
    }
...
void bot_minion_ai::CastBotItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto, Spell const* /*spell*//* = NULL*/)
{
    ...
    if (procVictim & PROC_FLAG_TAKEN_DAMAGE)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (proto->SpellPPMRate)//spellData.SpellPPMRate)
            {
                uint32 WeaponSpeed = me->GetAttackTime(attType);
                chance = me->GetPPMProcChance(WeaponSpeed, proto->SpellPPMRate/*spellData.SpellPPMRate*/, spellInfo);
            }
...
void bot_ai::BuildGrouUpdatePacket(WorldPacket* data)
{
	...
	data->append(me->GetPackGUID());//*data << me->GetPackGUID();
    *data << uint32(mask);
