    ...
    struct shaman_botAI : public bot_minion_ai
    {
        ...
        void DoNormalAttack(uint32 diff)
        {
                ...
                IsSpellReady(FLAME_SHOCK_1, diff) && HasRole(BOT_ROLE_DPS) && dist < 25 && Rand() < 30)
            {
                ...
                if (canFlameShock)
                {
                    if (Aura* fsh = opponent->GetAura(GetSpell(FLAME_SHOCK_1), me->GetGUID()).get())
        ...
        void DoNonCombatActions(uint32 diff)
        {
            ...
            {
                /*Aura**/AuraPtr shield = /*NULL*/nullptr;
        ...
        bool BuffTarget(Unit* target, uint32 diff)
        {
                ...
                me->GetExactDist(target) < 40 && Rand() < 75)
            {
                ...
                if (!cast)
                    if (Aura* eShield = target->GetAura(GetSpell(EARTH_SHIELD_1)).get())
        ...
        void OnBotDespawn(Creature* summon)
        {
            ...
            {
                sLog->outError(LOG_FILTER_PLAYER, "SummonedCreatureDespawn(): Shaman bot %s has despawned summon %s which is not a temp summon or not a totem...",
					me->GetName(), summon->GetName());
                return;
            }
        ...
        void OnBotSummon(Creature* summon)
        {
            ...
            switch (totem->m_Properties->Id)
            {
                ...
                case SUMMON_TYPE_TOTEM_AIR:
                    ...
                    break;
                default:
                    sLog->outError(LOG_FILTER_PLAYER, "OnBotSummon(): Shaman bot %s has summoned totem %s with unknown type %u", me->GetName(), summon->GetName(), totem->m_Properties->Id);
                    return;
            }
			...
			master->m_SummonSlot[++slot] = 0;// .Clear();
        }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell)
        {
            ...
            {
                if (Aura* mwb = me->GetAura(MAELSTROM_WEAPON_BUFF).get())
                {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            {
                if (Aura* shield = target->GetAura(spellId, me->GetGUID()).get())
                {
